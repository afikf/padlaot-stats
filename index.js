document.addEventListener('DOMContentLoaded', () => {
    // ===================================================================
    // == שים לב: זה הקישור שהבאת מהגיליון הפשוט,      ==
    // == ולא הקישור של גיליון הנתונים הגולמיים!        ==
    // == עליך לפרסם את "הזנת נתונים - משחקים" ולקבל קישור חדש. ==
    // ===================================================================
    const RAW_DATA_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQjSHec_HTnsbDypeWhzQZalUoOGvuPFsUhzung3nnM8cj9vIfeCgWf4KakONdwXC36XOQQ8ZuAwPlN/pub?gid=188811576&single=true&output=csv';
    ...
    let allGamesData = []; // To store the fetched raw data
    ...
    fetch(RAW_DATA_URL)
        ...
        .then(csvText => {
            ...
            // Important: This assumes the CSV structure of the RAW data sheet.
            // date, name, hasPlayed, games, goals, assists
            const rows = csvText.trim().split('\n').slice(1);

            allGamesData = rows.map(row => {
                const cells = row.split(',');
                if (cells.length < 6) return null; // Skip malformed rows
                
                const [date, name, hasPlayed, games, goals, assists] = cells;
                
                // Critical filter: Only include rows where a player actually played.
                if (!hasPlayed || hasPlayed.trim().toLowerCase() !== 'true') return null;
        ...
        .catch(error => {
            ...
            loader.style.display = 'none';
            lastUpdated.textContent = 'שגיאה בטעינת הנתונים.';
